...
interface StatusProps {
    handleClick :(event:React.MouseEvent<HTMLDivElement|SVGElement>)=>void,
    statusName:string,
    profilePictureUrl?:string,
    userCreateStatus?:boolean
}
export const Status :React.FC<StatusProps>= ({handleClick,statusName,profilePictureUrl,userCreateStatus = false})=>{
    return (
        ...
        <div className="text-center hover:cursor-pointer mx-4 " >
          ...
          `http://localhost:8000/media/${profilePictureUrl}`
           ...
           />
           { userCreateStatus &&
                <FaPlus onClick={(event) => handleClick(event)} className="absolute bottom-[-6px] right-3 md:right-0  text-base text-primary dark:text-secondary"/>
           }
                </div>
          </div>
