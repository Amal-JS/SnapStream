    ...
    const [commentsCount,setCommentsCount] = useState<number>(0)
      ...
      const fetchUserPosts = async ()=>{
        ...
        if(response.data.posts && response.status === 200){
            ...
            !response.data.posts.isUserCommentedOnPost && setIsPlaceholderVisible(true)
        }else{
    ...
    return (
        <CustomModal isDismissable={true} modalToggle={isModalOpened} size="4xl">
          ...
          (post.totalCommentsCount === 0  ) &&
              <p className="text-base text-primary dark:text-secondary mt-4 text-center">No comments added.</p>
        }


        { !post.isUserCommentedOnPost   && <div className="flex mt-3">
          <div
          ...
          >
            ...
            <div className="w-2/12 flex items-center ml-2">
              { isCommentAdded && <p className="font-bold text-small text-btn-enabled hover:cursor-pointer" onClick={handleNewCommentCreation}>Post</p> }
                           ...
                           <CommentDiv postId={post.id} handleCommentsCountChange={handleCommentsCountChange} postViewCommentUpdate={isCommentAdded}/>
